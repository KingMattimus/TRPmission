...
_market = missionNamespace getVariable "TroleMarketPrices";

_good = missionNamespace getVariable format["%1TroleMarketGoodPrice",_var];
...

{
    if(SEL(_x,1) == _factor) then {
        _name = format["%1TroleMarketGoodPrice",SEL(_x,0)];
