...
if (lbCurSel 1501 isEqualTo 0 AND lbCurSel 1502 > -1) then {
	_confirmed = true;
	_car setvariable ['GOM_fnc_NitroStage',lbCurSel 1502];
	_car setvariable ['GOM_fnc_nitroSize',lbCurSel 1502];
	_car setvariable ['GOM_fnc_NitroVolume',1];
	systemchat format ['%2: Nitro Stage %1 (%3l) installed.',lbCurSel 1502,_carName,(lbCurSel 1502) * 5];
	lbSetCurSel [1502, -1];
	lbClear 1502;
...
if (lbCurSel 1501 isEqualTo 1 AND lbCurSel 1502 > -1) then {
	_confirmed = true;
	_multi = _car getVariable ['GOM_fnc_MaxspeedMulti',[1,1,1,1]];
	_car setVariable ['GOM_fnc_MaxSpeed',(_multi select (lbCurSel 1502))];
	_car setvariable ['GOM_fnc_MaxSpeedType',lbCurSel 1502];
	systemchat format ['%2: Transmission Stage %1 installed.',lbCurSel 1502,_carName];
	lbSetCurSel [1502, -1];
...
if (lbCurSel 1501 isEqualTo 2 AND lbCurSel 1502 > -1) then {
	...
	_confirmed = true;
	_car setvariable ['GOM_fnc_brakeType',lbCurSel 1502];
...
if (lbCurSel 1501 isEqualTo 3 AND lbCurSel 1502 > -1) then {
	...
	_confirmed = true;
	_base = [1,0.9,0.8,0.7];
	_baseValue = _base select (lbCurSel 1502);
	_reduction = random [_baseValue - 0.02,_baseValue,_baseValue + 0.02];
	if (lbCurSel 1502 isEqualTo 0) then {_reduction = 1};
		_car setmass ((_car getvariable ['GOM_fnc_initMass',1500]) * _reduction);
		_car setvariable ['GOM_fnc_ChassisKit',1];
	...
	if (lbCurSel 1501 isEqualTo 4 AND lbCurSel 1502 > -1) then {
		...
		if (lbCurSel 1502 isEqualto 0) then {
			...
			_car setVariable ['GOM_fnc_CruiseControlInstalled',true];
		...
		if (lbCurSel 1502 isEqualto 1) then {
			...
			_car setVariable ['GOM_fnc_FogMachineInstalled',true];
			_car setVariable ['GOM_fnc_FogMachineRounds',3];
		...
		if (lbCurSel 1502 isEqualto 2) then {
			...
			_car setVariable ['GOM_fnc_EjectionSeatsInstalled',true];
